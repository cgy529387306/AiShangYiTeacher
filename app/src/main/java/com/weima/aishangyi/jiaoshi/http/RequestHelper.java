package com.weima.aishangyi.jiaoshi.http;
...
import com.mb.android.utils.common.BaseExtra;
...
public class RequestHelper {
	...
	private static RequestQueue sQueue = Volley.newRequestQueue(MBApplication.getInstance());
	...
	public static void request(final int method, final RequestEntity fEntity, final ResponseListener fListener, final Object... extras) {
		...
		fEntity.getTimeoutMs();
		String json = JsonHelper.toJson(fEntity.getRequestParamsMap());
		...
		if (Request.Method.GET == method && Helper.isNotEmpty(fEntity.getRequestParamsMap())) {
			// 转换参数列表为get请求格式
			List<BasicNameValuePair> nameValuePairs = new ArrayList<BasicNameValuePair>();
			for (String key : fEntity.getRequestParamsMap().keySet()) {
				nameValuePairs.add(new BasicNameValuePair(key, (String)fEntity.getRequestParamsMap().get(key)));
			}
			String urlEncoded = URLEncodedUtils.format(nameValuePairs, Charset.defaultCharset().name());
			// 获取原请求地址，在原请求地址末尾追加get参数
			String url = fEntity.getUrl();
			if (url.contains("?")) {
				if (url.endsWith("&")) {
			...
			request = new JsonObjectRequest(Request.Method.GET,fEntity.getUrl(), jsonObject,
					genSuccessListener(fEntity, fListener, extras),genErrorListener(fEntity, fListener, extras)){
				@Override
			...
			request = new JsonObjectRequest(Request.Method.POST,fEntity.getUrl(), jsonObject,
					genSuccessListener(fEntity, fListener, extras),genErrorListener(fEntity, fListener, extras)){
				@Override
				public Map<String, String> getHeaders() throws AuthFailureError {
					// 添加自定义header信息
	 ...
	 * @param mode 登陆方式 1、新浪脉搏 2、QQ，若无则为null
	...
	protected static String getDeviceInfo() {
		...
		StringBuilder deviceInfo = new StringBuilder()
				...
				.append("&openid=")
				.append(AppHelper.getDeviceUniqueString(MBApplication.getInstance()));
	...
	protected static String getProjectInfo() {
		...
		StringBuilder projectInfo = new StringBuilder()
				...
				// 浏览器标识
				.append("&ua=")
				.append(new WebView(MBApplication.getInstance()).getSettings().getUserAgentString());
	...
	public static int getNetWorkType() {
		ConnectivityManager connectionManager =
                (ConnectivityManager) MBApplication.getInstance().getSystemService(Context.CONNECTIVITY_SERVICE);
	...
	public static String getBssid() {
		WifiManager wm = (WifiManager) MBApplication.getInstance().getSystemService(Context.WIFI_SERVICE);
		if (Helper.isNull(wm)) {
			return "";
		}
	...
	private static Response.Listener<JSONObject> genSuccessListener(final RequestEntity fEntity, final ResponseListener fListener,
																final Object... fExtras) {
